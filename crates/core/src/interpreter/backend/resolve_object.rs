use crate::common::{
     chain::ChainOrRpc, object::{Object, ObjectField}, query_result:: ObjectQueryRes
};
use anyhow::Result;
...
async fn get_objects(
   ...
   let mut objects = Vec::new();
    ...
    let options = SuiObjectDataOptions::default().with_owner().with_previous_transaction().with_bcs().with_type();
    let results = provider.read_api().multi_get_object_with_options(objects_ids, options)
        .await?;
    for object in results.iter() { 
        let Some(object_data) = object.data.as_ref() else {continue;};
        let Some(owner_ref) = object_data.owner.as_ref() else {  continue;};
        let Ok(owner) = owner_ref.get_address_owner_address() else { continue;};
        ...
        for field in &fields {
            match field {
                ...
                ObjectField::Version =>  {
                            object_res.version = Some(object_data.version)
                        }
                }
        }
